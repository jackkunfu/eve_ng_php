...
function addMessage(severity, message, notFromLabviewport) {
  ...
  if ($('#lab-viewport').length) {
    if (!$('#alert_container').length) {
      ...
      $('body').append(
        '<div id="alert_container"><b><i class="fa fa-bell-o"></i> Notifications</b><div class="inner"></div></div>'
      );
    }
    var msgalert = $(
      '<div class="alert alert-' + severity.toLowerCase() + ' fade in">'
    )
      .append(
  ...
  if (
    $('#lab-viewport').length ||
    (!$('#lab-viewport').length && notFromLabviewport)
  ) {
...
function addModal(title, body, footer, prop) {
  var html =
    '<div aria-hidden="false" style="display: block;z-index: 10000;" class="modal ' +
    ' ' +
    prop +
    ' fade in" tabindex="-1" role="dialog"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">' +
    title +
...
function addModalError(message) {
  var html =
    '<div aria-hidden="false" style="display: block;" class="modal fade in" tabindex="-1" role="dialog"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">' +
    MESSAGES[15] +
    '</h4></div><div class="modal-body">' +
    message +
...
function addModalWide(title, body, footer, property) {
  ...
  if (
    title.toUpperCase() == 'STARTUP-CONFIGS' ||
    title.toUpperCase() == 'CONFIGURED NODES' ||
    title.toUpperCase() == 'CONFIGURED TEXT OBJECTS' ||
    title.toUpperCase() == 'CONFIGURED NETWORKS' ||
    title.toUpperCase() == 'CONFIGURED NODES' ||
    title.toUpperCase() == 'STATUS' ||
    title.toUpperCase() == 'PICTURES'
  ) {
    addittionalHeaderBtns =
      '<i title="Make transparent" class="glyphicon glyphicon-certificate pull-right action-changeopacity"></i>';
  }
  var html =
    '<div aria-hidden="false" data-backdrop="static" data-keyboard="false" style="display: block;" class="modal modal-wide ' +
    prop +
    ' fade in" tabindex="-1" role="dialog"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"></i><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>' +
    addittionalHeaderBtns +
...
function recursive_cfg_export(nodes, i) {
  ...
  addMessage(
    'info',
    nodes[Object.keys(nodes)[i]]['name'] + ': ' + MESSAGES[138]
  );
  var deferred = $.Deferred();
  var lab_filename = $('#lab-viewport').attr('data-path');
  if (typeof nodes[Object.keys(nodes)[i]]['path'] === 'undefined') {
    ...
    var url =
      ...
      lab_filename +
      '/nodes/' +
      nodes[Object.keys(nodes)[i]]['path'] +
      '/export';
  }
  logger(1, 'DEBUG: ' + url);
  var type = 'PUT';
  $.ajax({
    ...
    success: function (data) {
      if (data['status'] == 'success') {
        ...
        addMessage(
          'success',
          nodes[Object.keys(nodes)[i]]['name'] + ': ' + MESSAGES[79]
        );
      } else {
        // Application error
        logger(
        ...
        addMessage(
          'danger',
          nodes[Object.keys(nodes)[i]]['name'] + ': ' + data['message']
        );
      }
      if (i > 0) {
    ...
    error: function (data) {
      ...
      addMessage(
        'danger',
        nodes[Object.keys(nodes)[i]]['name'] + ': ' + message
      );
      if (i > 0) {
        recursive_cfg_export(nodes, i);
      } else {
  ...
  $('form :input[name^="' + form_name + '["]').each(function (id, object) {
    ...
    form_array[
      $(this)
        ...
        .substr(
          form_name.length + 1,
          $(this).attr('name').length - form_name.length - 2
        )
    ] = $(this).val();
  });
  return form_array;
...
function form2ArrayByRow(form_name, id) {
  ...
  $('form :input[name^="' + form_name + '["][data-path="' + id + '"]').each(
    function (id, object) {
      // INPUT name is in the form of "form_name[value]", get value only
      form_array[
        $(this)
...
function getJsonMessage(response) {
  ...
  try {
    ...
    if (response != '') {
      ...
      message =
        'Undefined message, check if the UNetLab VM is powered on. If it is, see <a href="/Logs" target="_blank">logs</a>.';
    }
  }
  return message;
}
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data['status'] == 'success') {
        logger(
          1,
          'DEBUG: got available links(s) from lab "' + lab_filename + '".'
        );
        deferred.resolve(data['data']);
      } else {
        // Application error
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data['status'] == 'success') {
        logger(
          1,
          'DEBUG: got sartup-config(s) from lab "' + lab_filename + '".'
        );
        deferred.resolve(data['data']);
      } else {
        // Application error
    ...
    var request = $.ajax({
      ...
      success: function (data) {
        if (data['status'] == 'success') {
          ...
          // Close the modal
          $('#node' + id + ' .node_name').html(
            '<i class="node' +
              id +
              '_status glyphicon glyphicon-stop"></i>' +
              form_data['name']
          );
          $('#node' + id + ' a img').attr(
            'src',
            '/images/icons/' + form_data['icon']
          );
          addMessage(data['status'], data['message']);
        } else {
          // Application error
...
function recursive_start(nodes, i) {
  ...
  if (typeof nodes[Object.keys(nodes)[i]]['path'] === 'undefined') {
    var url =
      '/api/labs' + lab_filename + '/nodes/' + Object.keys(nodes)[i] + '/start';
  } else {
    var url =
      '/api/labs' +
      lab_filename +
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data['status'] == 'success') {
        ...
        addMessage(
          'success',
          nodes[Object.keys(nodes)[i]]['name'] + ': ' + MESSAGES[76]
        );
        ...
        addMessage(
          'danger',
          nodes[Object.keys(nodes)[i]]['name'] + ': ' + MESSAGES[76] + 'failed'
        );
      }
      if (i > 0) {
...
function printContextMenu(title, body, pageX, pageY, addToBody) {
  ...
  menu +=
    '<ul class="dropdown-menu" role="menu"><li role="presentation" class="dropdown-header">' +
    title +
    '</li>' +
    body +
    '</ul></div>';
...
function printFormFolder(action, values) {
  ...
  if (original == '/' && action == 'rename') {
    ...
    var html =
      '<form id="form-folder-' +
      action +
      '" class="form-horizontal form-folder-' +
      action +
      '"><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[20] +
      '</label><div class="col-md-5"><input class="form-control" name="folder[path]" value="' +
      path +
      '" disabled type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[19] +
      '</label><div class="col-md-5"><input class="form-control autofocus" name="folder[name]" value="' +
      name +
      '" type="text"/></div></div><div class="form-group"><div class="col-md-5 col-md-offset-3"><input class="form-control" name="folder[original]" value="' +
      original +
      '" type="hidden"/><button type="submit" class="btn btn-success">' +
      submit +
      '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
      MESSAGES[18] +
      '</button></div></div></form>';
...
function printFormImport(path) {
  var html =
    '<form id="form-import" class="form-horizontal form-import"><div class="form-group"><label class="col-md-3 control-label">' +
    MESSAGES[20] +
    '</label><div class="col-md-5"><input class="form-control" name="import[path]" value="' +
    path +
    '" disabled type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
    MESSAGES[2] +
    '</label><div class="col-md-5"><input class="form-control" name="import[local]" value="" disabled="" placeholder="' +
    MESSAGES[25] +
    '" "type="text"/></div></div><div class="form-group"><div class="col-md-7 col-md-offset-3"><span class="btn btn-default btn-file btn-success">' +
    MESSAGES[23] +
    ' <input class="form-control" name="import[file]" value="" type="file"></span> <button type="submit" class="btn btn-flat">' +
    MESSAGES[24] +
    '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
    MESSAGES[18] +
    '</button></div></div></form>';
...
function printFormLab(action, values) {
  if (action == 'add') {
    ...
    var path =
      values['path'] == '/'
        ? '/' + values['name'] + '.unl'
        : values['path'] + '/' + values['name'] + '.unl';
  }
  var name = values['name'] != null ? values['name'] : '';
  var version = values['version'] != null ? values['version'] : '';
  var scripttimeout =
    values['scripttimeout'] != null ? values['scripttimeout'] : '300';
  ...
  var html =
    '<form id="form-lab-' +
    action +
    '" class="form-horizontal form-lab-' +
    action +
    '">' +
    ...
    '</label>' +
    '<div class="col-md-5">' +
    '<input class="form-control" name="lab[path]" value="' +
    path +
    '" disabled="" type="text"/>' +
    '</div>' +
    '</div>' +
    ...
    MESSAGES[19] +
    '</label>' +
    '<div class="col-md-5"><input class="form-control autofocus" name="lab[name]" value="' +
    name +
    '" type="text"/>' +
    '</div>' +
    '</div>' +
    ...
    MESSAGES[26] +
    '</label>' +
    '<div class="col-md-5"><input class="form-control" name="lab[version]" value="' +
    version +
    '" type="text"/>' +
    '</div>' +
    '</div>' +
    ...
    '<div class="col-md-5">' +
    '<input class="form-control" name="lab[author]" value="' +
    author +
    '" type="text"/>' +
    '</div>' +
    '</div>' +
    ...
    '</label>' +
    '<div class="col-md-5">' +
    '<textarea class="form-control" name="lab[description]">' +
    description +
    '</textarea>' +
    '</div>' +
    '</div>' +
    ...
    '</label>' +
    '<div class="col-md-5">' +
    '<textarea class="form-control" name="lab[body]">' +
    body +
    '</textarea>' +
    '</div>' +
    '</div>' +
    ...
    MESSAGES[158] +
    '</label>' +
    '<div class="col-md-5"><input class="form-control" name="lab[scripttimeout]" value="' +
    scripttimeout +
    '" type="text"/>' +
    '</div>' +
    '</div>' +
...
function printFormNetwork(action, values) {
  ...
  $.when(getNetworkTypes()).done(function (network_types) {
    ...
    var html =
      '<form id="form-network-' + action + '" class="form-horizontal">';
    if (action == 'add') {
      // If action == add -> print the nework count input
      html +=
        '<div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[114] +
        '</label><div class="col-md-5"><input class="form-control" name="network[count]" value="1" type="text"/></div></div>';
    } else {
      // If action == edit -> print the network ID
      html +=
        '<div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[92] +
        '</label><div class="col-md-5"><input class="form-control" disabled name="network[id]" value="' +
        id +
        '" type="text"/></div></div>';
    }
    html +=
      '<div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[103] +
      '</label><div class="col-md-5"><input class="form-control autofocus" name="network[name]" value="' +
      name +
      '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[95] +
      '</label><div class="col-md-5"><select class="selectpicker show-tick form-control" name="network[type]" data-live-search="true" data-style="selectpicker-button">';
    ...
    html +=
      '</select></div></div><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[93] +
      '</label><div class="col-md-5"><input class="form-control" name="network[left]" value="' +
      left +
      '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[94] +
      '</label><div class="col-md-5"><input class="form-control" name="network[top]" value="' +
      top +
      '" type="text"/></div></div><div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
      MESSAGES[47] +
      '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
      MESSAGES[18] +
      '</button></div></div></form></form>';
...
function printFormNode(action, values) {
  ...
  var template =
    values == null || values['template'] == null ? null : values['template'];

  var title = action == 'add' ? MESSAGES[85] : MESSAGES[86];
  var template_disabled =
    values == null || values['template'] == null ? '' : 'disabled ';

  $.when(getTemplates(null))
    .done(function (templates) {
      var html = '';
      html +=
        '<form id="form-node-' +
        action +
        '" class="form-horizontal"><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[84] +
        '</label><div class="col-md-5"><select id="form-node-template" class="selectpicker form-control" name="node[template]" data-live-search="true" data-style="selectpicker-button"><option value="">' +
        MESSAGES[102] +
        '</option>';
      ...
      html +=
        '</select></div></div><div id="form-node-data"></div><div id="form-node-buttons"></div></form>';
      ...
      $('#form-node-template').change(function (e2) {
        ...
        if (template != '') {
          ...
          $.when(getTemplates(template), getNodes(id))
            .done(function (template_values, node_values) {
              // TODO: this event is called twice
              id = id == null ? '' : id;
              var html_data =
                '<input name="node[type]" value="' +
                template_values['type'] +
                '" type="hidden"/>';
              if (action == 'add') {
                // If action == add -> print the nework count input
                html_data +=
                  '<div class="form-group"><label class="col-md-3 control-label">' +
                  MESSAGES[113] +
                  '</label><div class="col-md-5"><input class="form-control" name="node[count]" value="1" type="text"/></div></div>';
              } else {
                // If action == edit -> print the network ID
                html_data +=
                  '<div class="form-group"><label class="col-md-3 control-label">' +
                  MESSAGES[92] +
                  '</label><div class="col-md-5"><input class="form-control" disabled name="node[id]" value="' +
                  id +
                  '" type="text"/></div></div>';
              }
              $.each(template_values['options'], function (key, value) {
                ...
                var value_set =
                  node_values != null && node_values[key] != null
                    ? node_values[key]
                    : value['value'];
                if (value['type'] == 'list') {
                  // Option is a list
                  html_data +=
                    '<div class="form-group"><label class="col-md-3 control-label">' +
                    value['name'] +
                    '</label><div class="col-md-5"><select class="selectpicker form-control" name="node[' +
                    key +
                    ']" data-style="selectpicker-button">';
                  $.each(value['list'], function (list_key, list_value) {
                    var selected = list_key == value_set ? 'selected ' : '';
                    iconstyle = '';
                    if (key == 'icon') {
                      iconstyle =
                        'style="background-image:url(/images/icons/' +
                        list_value +
                        ');"';
                    }
                    html_data +=
                  ...
                  html_data +=
                    '<div class="form-group"><label class="col-md-3 control-label">' +
                    value['name'] +
                    '</label><div class="col-md-5"><input class="form-control' +
                    (key == 'name' ? ' autofocus' : '') +
                    '" name="node[' +
                    key +
                    ']" value="' +
              ...
              html_data +=
                '<div class="form-group"><label class="col-md-3 control-label">' +
                MESSAGES[93] +
                '</label><div class="col-md-5"><input class="form-control" name="node[left]" value="' +
                left +
                '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
                MESSAGES[94] +
                '</label><div class="col-md-5"><input class="form-control" name="node[top]" value="' +
                top +
                '" type="text"/></div></div>';

              // Show the buttons
              $('#form-node-buttons').html(
                '<div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
                  MESSAGES[47] +
                  '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
                  MESSAGES[18] +
                  '</button></div>'
...
function printFormNodeConfigs(values, cb) {
  ...
  if (ROLE == 'admin' || ROLE == 'editor') {
    var html =
      '<form id="form-node-config" class="form-horizontal"><input name="config[id]" value="' +
      values['id'] +
      '" type="hidden"/><div class="form-group"><div class="col-md-12"><textarea class="form-control autofocus" id="nodeconfig" name="config[data]" rows="15"></textarea></div></div><div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
      MESSAGES[47] +
      '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
      MESSAGES[18] +
      '</button></div></div></form>';
...
function printFormCustomShape(values) {
  ...
  var html =
    '<form id="main-modal" class="container col-md-12 col-lg-12 custom-shape-form">' +
    '<div class="row">' +
    ...
    '</button>' +
    '</div>' +
    '<input  type="text" class="hide left-coordinate" value="' +
    left +
    '">' +
    '<input  type="text" class="hide top-coordinate" value="' +
    top +
    '">' +
    '</form>';
  ...
  for (var i = 0; i < shapeTypes.length; i++) {
    $('.shape-type-select').append(
      $('<option></option>').val(shapeTypes[i]).html(shapeTypes[i])
    );
  }

  for (var j = 0; j < borderTypes.length; j++) {
    $('.border-type-select').append(
      $('<option></option>').val(borderTypes[j]).html(borderTypes[j])
    );
  }
}
...
function printFormText(values) {
  ...
  var html =
    '<form id="main-modal" class="container col-md-12 col-lg-12 add-text-form">' +
    '<div class="row">' +
  ...
  for (var i = 0; i < fontStyles.length; i++) {
    $('.text-font-style-select').append(
      $('<option></option>').val(fontStyles[i]).html(fontStyles[i])
    );
  }
}
      ...
      if (values['sort'] == 'iol') {
        ...
        $.each(values['ethernet'], function (interfc_id, interfc) {
          ...
          iol_interfc[4 * x + y] =
            '<div class="form-group"><label class="col-md-3 control-label">' +
            interfc['name'] +
            '</label><div class="col-md-5"><select class="selectpicker form-control" name="interfc[' +
            interfc_id +
            ']" data-live-search="true" data-style="selectpicker-button"><option value="">' +
            MESSAGES[117] +
            '</option>';
          $.each(links['ethernet'], function (link_id, link) {
            var link_selected =
              interfc['network_id'] == link_id ? 'selected ' : '';
            iol_interfc[4 * x + y] +=
              '<option ' +
              link_selected +
              'value="' +
          ...
          $.each(links['serial'], function (node_id, serial_link) {
            if (values['node_id'] != node_id) {
              $.each(serial_link, function (link_id, link) {
                var link_selected =
                  interfc['remote_id'] + ':' + interfc['remote_if'] ==
                  node_id + ':' + link_id
                    ? 'selected '
                    : '';
                iol_interfc[4 * x + y] +=
  ...
  $.when(getPicturesMapped(picture_id))
    .done(function (picture) {
      ...
      var body =
        ...
        picture['height'] +
        '"/>' +
        '<map name="picture_map">' +
        picture_map +
        '</map>' +
        '</div>';
      if (ROLE == 'admin' || ROLE == 'editor') {
        var footer =
          '<button type="button" class="btn btn-flat action-pictureedit" data-path="' +
          picture_id +
          '">Edit</button>';
      } else {
        var footer = '';
      ...
      printNodesMap(
        { name: picture['name'], body: body, footer: footer },
        function () {
          setTimeout(function () {
            $('map').imageMapResize();
          }, 500);
...
function displayPictureForm(picture_id) {
  ...
  if (picture_id == null) {
    ...
    // Header
    form +=
      '<form id="form-' + action + '" class="form-horizontal form-picture">';
    // Name
    form +=
      '<div class="form-group"><label class="col-md-3 control-label">Name</label><div class="col-md-5"><input type="text" class="form-control-static" name="picture[name]" value=""/></div></div>';
    // File (add only)
    form +=
      '<div class="form-group"><label class="col-md-3 control-label">Picture</label><div class="col-md-5"><input type="file" name="picture[file]" value=""/></div></div>';
    // Footer
    form +=
      '<div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
      button +
      '</button><button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button></div></div></form>';
      ...
      if (picture != null) {
        if ($(location).attr('pathname') == '/lab_edit.php') {
          ...
          form +=
            ...
            action +
            '" tabindex="-1" role="dialog"><div class="modal-dialog" style="width: 100%;"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">' +
            title +
            '</h4></div><div class="modal-body"><form id="form-' +
            action +
            '" class="form-horizontal form-picture">';
          // Name
          form +=
            '<div class="form-group"><label class="col-md-3 control-label">Name</label><div class="col-md-5"><input type="text" class="form-control" name="picture[name]" value="' +
            picture_name +
            '"/></div></div>';
          // Picure
          form +=
            '<img id="lab_picture" src="/api/labs' +
            lab_file +
            '/pictures/' +
            picture_id +
            '/data">';
          // MAP
          form +=
            '<div class="form-group"><label class="col-md-3 control-label">Map</label><div class="col-md-5"><textarea type="textarea" name="picture[map]">' +
            picture_map +
            '</textarea></div></div>';
          // Footer
          form +=
          ...
          form +=
            '<div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
            button +
            '</button> <button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button></div></div></form></div></div></div></div>';
          // Add the form to the HTML page
          $('#form_frame').html(form);
          ...
          form +=
            ...
            '/pictures/' +
            picture_id +
            '/data" usemap="#picture_map">';
          // Map
          form +=
            '<map name="picture_map">' + translateMap(picture_map) + '</map>';
          // Footer
        ...
        raiseMessage(
          'DANGER',
          'Cannot get picture (picture_id = ' + picture_id + ').'
        );
        deferred.reject();
      }
    });
...
function printFormPicture(action, values) {
  ...
  if (action == 'add') {
    html +=
      ...
      '"><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[19] +
      '</label><div class="col-md-5"><input class="form-control" autofocus name="picture[name]" value="' +
      name +
      '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[137] +
      '</label><div class="col-md-5"><textarea class="form-control" name="picture[map]">' +
      map +
      '</textarea></div></div><div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
      MESSAGES[47] +
      '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
      MESSAGES[18] +
      '</button></div></div></form>';
    ...
    html +=
      ...
      '" data-path=' +
      values['id'] +
      '><div class="follower-wrapper"><img src="/api/labs' +
      $('#lab-viewport').attr('data-path') +
      '/pictures/' +
      values['id'] +
      '/data" alt="' +
      ...
      '" height="' +
      values['height'] +
      '"/><div id="follower"></div></div><div class="form-group"><label class="col-md-3 control-label">' +
      MESSAGES[19] +
      '</label><div class="col-md-5"><input class="form-control" autofocus name="picture[name]" value="' +
      name +
      '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
...
function printFormUser(action, values) {
  $.when(getRoles())
    .done(function (roles) {
      ...
      var expiration =
        values['expiration'] != null && values['expiration'] != -1
          ? $.datepicker.formatDate(
              'yy-mm-dd',
              new Date(values['expiration'] * 1000)
            )
          : '';
      var pod =
        values['pod'] != null && values['pod'] != -1 ? values['pod'] : '';
      var pexpiration =
        values['pexpiration'] != null && values['pexpiration'] != -1
          ? $.datepicker.formatDate(
              'yy-mm-dd',
              new Date(values['pexpiration'] * 1000)
            )
          : '';
      var submit = action == 'add' ? MESSAGES[17] : MESSAGES[47];
      var title =
        action == 'add' ? MESSAGES[34] : MESSAGES[48] + ' ' + username;
      var user_disabled = action == 'add' ? '' : 'disabled ';
      var html =
        '<form id="form-user-' +
        action +
        '" class="form-horizontal form-user-' +
        action +
        '"><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[44] +
        '</label><div class="col-md-5"><input class="form-control autofocus" ' +
        user_disabled +
        'name="user[username]" value="' +
        username +
        '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[19] +
        '</label><div class="col-md-5"><input class="form-control" name="user[name]" value="' +
        name +
        '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[28] +
        '</label><div class="col-md-5"><input class="form-control" name="user[email]" value="' +
        email +
        '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[45] +
        '</label><div class="col-md-5"><input class="form-control" name="user[password]" value="" type="password"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[29] +
        '</label><div class="col-md-5"><select class="selectpicker show-tick form-control" name="user[role]" data-live-search="true" data-style="selectpicker-button">';
      ...
      html +=
        '</select></div></div><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[30] +
        '</label><div class="col-md-5"><input class="form-control expiration" name="user[expiration]" value="' +
        expiration +
        '" type="text"/></div></div><h4>' +
        MESSAGES[46] +
        '</h4><div class="form-group"><label class="col-md-3 control-label">POD</label><div class="col-md-5"><input class="form-control pod" name="user[pod]" value="' +
        pod +
        '" type="text"/></div></div><div class="form-group"><label class="col-md-3 control-label">' +
        MESSAGES[30] +
        '</label><div class="col-md-5"><input class="form-control expiration pod" name="user[pexpiration]" value="' +
        pexpiration +
        '" type="text"/></div></div><div class="form-group"><div class="col-md-5 col-md-offset-3"><button type="submit" class="btn btn-success">' +
        submit +
        '</button> <button type="button" class="btn btn-flat" data-dismiss="modal">' +
      ...
      addModal(title, html, '');
      if (ROLE == 'user') {
        $('#form-user-edit input,#form-user-edit select').prop(
          'disabled',
          true
        );
      ...
      if (ROLE == 'editor') {
        ...
        $('#form-user-edit .pod,#form-user-edit .expiration').prop(
          'disabled',
          true
        );
      }
...
function printLabTopology() {
  ...
  $.when(getNetworks(null), getNodes(null), getTopology(), getTextObjects())
    .done(function (networks, nodes, topology, textObjects) {
      ...
      progressbarMax =
        ...
        Object.keys(textObjects).length;
      ...
      $.each(networks, function (key, value) {
        ...
        $labViewport.append(
          '<div id="network' +
            value['id'] +
            '" ' +
            'class="context-menu network network' +
            value['id'] +
            ' network_frame ' +
            unusedClass +
            ' " ' +
        ...
        $('.progress-bar').css(
          'width',
          (++progressbarValue / progressbarMax) * 100 + '%'
        );
      });
      $.each(nodes, function (key, value) {
        if (value['url'].indexOf('token') != -1) {
          hrefbuf =
            '<a href="' + value['url'] + '" target="' + value['name'] + '" >';
        } else {
        ...
        $labViewport.append(
          '<div id="node' +
            ...
            hrefbuf +
            '</a>' +
            '<div class="node_name"><i class="node' +
            value['id'] +
            '_status"></i> ' +
            value['name'] +
            '</div>' +
      ...
      $.each(textObjects, function (key, value) {
        getTextObject(value['id'])
          .done(function (textObject) {
            $('.progress-bar').css(
              'width',
              (++progressbarValue / progressbarMax) * 100 + '%'
            );
          ...
          .fail(function () {
            logger(
              1,
              'DEBUG: Failed to load Text Object' + value['name'] + '!'
            );
          });
      });
      ...
      $.when
        .apply($, networkImgs.concat(nodesImgs))
        .done(function () {
          // Drawing topology
          jsPlumb.ready(function () {
            ...
            $(
              '._jsPlumb_connector, ._jsPlumb_overlay, ._jsPlumb_endpoint_anchor_'
            )
              .detach()
              .appendTo('#lab-viewport');
  ...
  $.when(labNodesResolver, labTextObjectsResolver)
    .done(function () {
  ...
  $.when(getNodes(null))
    .done(function (nodes) {
      $.each(nodes, function (node_id, node) {
        if (node['status'] == 0) {
          // Stopped
          $('.node' + node['id'] + '_status').attr(
            'class',
            'node' + node['id'] + '_status glyphicon glyphicon-stop'
          );
          ...
          $('.node' + node['id'] + '_status').attr(
            'class',
            'node' + node['id'] + '_status glyphicon glyphicon-warning-sign'
          );
          $('#node' + node['id'] + ' img').addClass('grayscale');
          ...
          $('.node' + node['id'] + '_status').attr(
            'class',
            'node' + node['id'] + '_status glyphicon glyphicon-play'
          );
          ...
          $('.node' + node['id'] + '_status').attr(
            'class',
            'node' + node['id'] + '_status glyphicon glyphicon-time'
          );
          $('#node' + node['id'] + ' img').removeClass('grayscale');
        } else {
          // Undefined
          $('.node' + node['id'] + '_status').attr(
            'class',
            'node' + node['id'] + '_status glyphicon glyphicon-question-sign'
          );
          $('#node' + node['id'] + ' img').addClass('grayscale');
        }
...
function printListNetworks(networks) {
  ...
  $.each(networks, function (key, value) {
    body +=
      ...
      '</td><td>' +
      value['count'] +
      '</td><td><a class="action-networkedit" data-path="' +
      value['id'] +
      '" data-name="' +
      value['name'] +
      '" href="javascript:void(0)" title="' +
      MESSAGES[71] +
      '"><i class="glyphicon glyphicon-edit"></i></a><a class="action-networkdelete" data-path="' +
      value['id'] +
      '" data-name="' +
      value['name'] +
      '" href="javascript:void(0)" title="' +
...
function checkTemplateValue(template_options, field) {
  if (template_options[field]) {
    ...
    parseInt(template_options[field]) === 0
  ) {
    return template_options[field].value.toString();
  } else {
    return '';
...
function createNodeListRow(template, id) {
  ...
  $.when(getTemplates(template), getNodes(id))
    .done(function (template_values, node_values) {
      ...
      var value_console =
        checkTemplateValue(template_values, 'console') ||
        node_values['console'] ||
        '';
      var value_serial = '';
      ...
      id = id == null ? '' : id;
      var html_data =
        '<tr><input name="node[type]" data-path="' +
        id +
        '" value="' +
        template_values['type'] +
        '" type="hidden"/>';
      html_data +=
        '<input name="node[left]" data-path="' +
        id +
        '" value="' +
        node_values['left'] +
        '" type="hidden"/>';
      html_data +=
        '<input name="node[top]" data-path="' +
        id +
        '" value="' +
        node_values['top'] +
        '" type="hidden"/>';
      ...
      // node id
      html_data +=
        '<td><input class="hide-border" style="width: 20px;" value="' +
        id +
        '" readonly/></td>';

      //node name
      html_data +=
        '<td><input class="configured-nodes-input" data-path="' +
        id +
      ...
      //node template
      html_data +=
        '<td><input class="hide-border" data-path="' +
        id +
      ...
      if (template == 'vpcs') {
        html_data +=
          '<td><input class="configured-nodes-input short-input readonly" data-path="' +
          id +
          '" name="node[cpu]" value="n/a" type="text" readonly /></td>';
      } else {
        html_data +=
          '<td><select class="configured-nods-select form-control" data-path="' +
          id +
          '" name="node[image]">';
        value_set =
          node_values != null &&
          template_values['options']['image'] &&
          template_values['options']['image']['list']
            ? node_values['image']
            : '';
        var options_arr =
          template_values['options']['image'] &&
      ...
      html_data +=
        '<td><input class="configured-nodes-input short-input ' +
        readonlyAttr +
        '" data-path="' +
        id +
        '" name="node[cpu]" value="' +
      ...
      html_data +=
        '<td><input class="configured-nodes-input ' +
        readonlyAttr +
        '" data-path="' +
        id +
        '" name="node[idlepc]" value="' +
        value_idlepc +
        '" type="text" ' +
        readonlyAttr +
        ' /></td>';
      ...
      if (template == 'vpcs') {
        ...
        readonlyAttr =
          value_ethernet && value_ethernet != 'n/a' ? '' : 'readonly';
      }
      html_data +=
        '<td><input class="configured-nodes-input short-input ' +
        readonlyAttr +
      ...
      if (template == 'iol') {
        html_data +=
          '<td><input class="hide-border"  data-path="' +
          id +
          '" value="telnet" readonly/></td>';
      } else if (template_values['options']['console']) {
        html_data +=
          '<td><select class="configured-nods-select form-control" name="node[console]" data-path="' +
          id +
          '" >';
        value_set =
          node_values != null && node_values['console'] != null
            ? node_values['console']
            : value['value'];
        $.each(
          template_values['options']['console']['list'],
        ...
        html_data +=
          '<td><input class="hide-border" name="node[console]" value="' +
          value_console +
      ...
      html_data +=
        ...
        '" name="node[icon]" >';
      value_set =
        node_values != null && node_values['icon'] != null
          ? node_values['icon']
          : value['value'];
      $.each(
        template_values['options']['icon']['list'],
        function (list_key, list_value) {
          var selected = list_key == value_set ? 'selected ' : '';
          html_data +=
            '<option ' +
      ...
      html_data +=
        ...
        '" name="node[config]">';
      value_set =
        node_values != null && node_values['config'] != null
          ? node_values['config']
          : value['value'];
      $.each(
        template_values['options']['config']['list'],
        function (list_key, list_value) {
          var selected = list_key == value_set ? 'selected ' : '';
          html_data +=
            '<option ' +
      ...
      html_data +=
        ...
        '<a class="action-nodestart" data-path="' +
        id +
        '" data-name="' +
        checkTemplateValue(template_values['options'], 'name') +
        '" href="javascript:void(0)" title="' +
        MESSAGES[66] +
        '"><i class="glyphicon glyphicon-play"></i></a>' +
        '<a class="action-nodestop" data-path="' +
        id +
        '" data-name="' +
        checkTemplateValue(template_values['options'], 'name') +
        '" href="javascript:void(0)" title="' +
        ...
        '<a class="action-nodewipe" data-path="' +
        id +
        '" data-name="' +
        checkTemplateValue(template_values['options'], 'name') +
        '" href="javascript:void(0)" title="' +
      ...
      if (ROLE == 'admin' || ROLE == 'editor') {
        html_data +=
          '<a class="action-nodeexport" data-path="' +
          id +
          '" data-name="' +
          checkTemplateValue(template_values['options'], 'name') +
          '" href="javascript:void(0)" title="' +
          ...
          '<a class="action-nodeinterfaces" data-path="' +
          id +
          '" data-name="' +
          checkTemplateValue(template_values['options'], 'name') +
          '" href="javascript:void(0)" title="' +
          ...
          '<a class="action-nodeedit" data-path="' +
          id +
          '" data-name="' +
          checkTemplateValue(template_values['options'], 'name') +
          '" href="javascript:void(0)" title="' +
          ...
          '<a class="action-nodedelete" data-path="' +
          id +
          '" data-name="' +
          checkTemplateValue(template_values['options'], 'name') +
          '" href="javascript:void(0)" title="' +
...
function printListNodes(nodes) {
  ...
  var body =
    '<div class="table-responsive"><form id="form-node-edit-table" ><table class="configured-nodes table"><thead><tr><th>' +
    MESSAGES[92] +
    '</th><th>' +
    MESSAGES[19] +
    '</th><th>' +
  ...
  var composePromise = function (key, value) {
    ...
    $.when(createNodeListRow(value['template'], value['id'])).done(function (
      data
    ) {
      html_rows.push(data);
    ...
    success: function (data) {
      ...
      if (data['status'] == 'success') {
        ...
        $.each(data['data']['folders'], function (id, object) {
          $('#list-folders > ul').append(
            '<li><a class="folder action-folderopen" data-path="' +
              object['path'] +
              '" href="javascript:void(0)" title="Double click to open, single click to select.">' +
              object['name'] +
              '</a></li>'
          );
        });
        ...
        $.each(data['data']['labs'], function (id, object) {
          $('#list-labs > ul').append(
            '<li><a class="lab action-labpreview" data-path="' +
              object['path'] +
              '" href="javascript:void(0)" title="Double click to open, single click to select.">' +
              object['file'] +
              '</a></li>'
        ...
        if (ROLE == 'admin' || ROLE == 'editor') {
          ...
          $('#actions-menu').empty();
          $('#actions-menu').append(
            '<li><a class="action-folderadd" href="javascript:void(0)"><i class="glyphicon glyphicon-folder-close"></i> ' +
              MESSAGES[4] +
              '</a></li>'
          );
          $('#actions-menu').append(
            '<li><a class="action-labadd" href="javascript:void(0)"><i class="glyphicon glyphicon-file"></i> ' +
              MESSAGES[5] +
              '</a></li>'
          );
          $('#actions-menu').append(
            '<li><a class="action-selectedclone" href="javascript:void(0)"><i class="glyphicon glyphicon-copy"></i> ' +
              MESSAGES[6] +
              '</a></li>'
          );
          $('#actions-menu').append(
            '<li><a class="action-selectedexport" href="javascript:void(0)"><i class="glyphicon glyphicon-export"></i> ' +
              MESSAGES[8] +
              '</a></li>'
          );
          $('#actions-menu').append(
            '<li><a class="action-import" href="javascript:void(0)"><i class="glyphicon glyphicon-import"></i> ' +
              MESSAGES[9] +
              '</a></li>'
          );
          $('#actions-menu').append(
            '<li><a class="action-folderrename" href="javascript:void(0)"><i class="glyphicon glyphicon-pencil"></i> ' +
              MESSAGES[10] +
              '</a></li>'
          );
          $('#actions-menu').append(
            '<li><a class="action-selecteddelete" href="javascript:void(0)"><i class="glyphicon glyphicon-trash"></i> ' +
              MESSAGES[7] +
              '</a></li>'
          );
          ...
          $('.folder').droppable({
            drop: function (e, o) {
              ...
              if (o['draggable'].hasClass('lab')) {
                $.when(moveLab(object, path))
                  .done(function (data) {
                    logger(
                      1,
                      'DEBUG: "' + object + '" moved to "' + path + '".'
                    );
...
function printUserManagement() {
  $.when(getUsers(null))
    .done(function (data) {
      var html =
        '<div class="row"><div id="users" class="col-md-12 col-lg-12"><div class="table-responsive"><table class="table"><thead><tr><th>' +
        MESSAGES[44] +
        '</th><th>' +
        MESSAGES[19] +
        '</th><th>' +
      ...
      html +=
        '<div class="row"><div id="pods" class="col-md-12 col-lg-12"><div class="table-responsive"><table class="table"><thead><tr><th>' +
        MESSAGES[44] +
        '</th><th>' +
        MESSAGES[32] +
        '</th><th>' +
      ...
      if (ROLE == 'admin') {
        ...
        $('#actions-menu').empty();
        $('#actions-menu').append(
          '<li><a class="action-useradd" href="javascript:void(0)"><i class="glyphicon glyphicon-plus"></i> ' +
            MESSAGES[34] +
            '</a></li>'
        );
        $('#actions-menu').append(
        ...
        $('#actions-menu').empty();
        $('#actions-menu').append(
          '<li><a href="javascript:void()">&lt;' + MESSAGES[3] + '&gt;</a></li>'
        );
      }
      ...
      $.each(data, function (id, object) {
        ...
        $('#users tbody').append(
          '<tr class="action-useredit user" data-path="' +
            username +
            '"><td class="username">' +
            username +
...
function drawStatusInModal(data) {
  ...
  $('#actions-menu', $statusModalBody).append(
    '<li><a class="action-sysstatus" href="javascript:void(0)"><i class="glyphicon glyphicon-refresh"></i> ' +
      MESSAGES[40] +
      '</a></li>'
  );
  $('#actions-menu', $statusModalBody).append(
    '<li><a class="action-stopall" href="javascript:void(0)"><i class="glyphicon glyphicon-stop"></i> ' +
      MESSAGES[50] +
      '</a></li>'
  );
  ...
  $('#stats-text ul', $statusModalBody).append(
    '<li>' + MESSAGES[39] + ': <code>' + data['version'] + '</code></li>'
  );
  $('#stats-text ul', $statusModalBody).append(
    '<li>' + MESSAGES[49] + ': <code>' + data['qemu_version'] + '</code></li>'
  );
  $('#stats-text ul', $statusModalBody).append(
    '<li>' + MESSAGES[165] + ': <code>' + data['uksm'] + '</code></li>'
  );
  $('#stats-text ul', $statusModalBody).append(
    '<li>' + MESSAGES[29] + ': <code>' + ROLE + '</code></li>'
  );
  $('#stats-text ul', $statusModalBody).append(
    '<li>' +
      MESSAGES[32] +
      ': <code>' +
      (TENANT == -1 ? 'none' : TENANT) +
      '</code></li>'
  );
  ...
  // CPU usage
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="circle circle-cpu col-md-3 col-lg-3"><strong></strong><br/><span>' +
      MESSAGES[36] +
      '</span></div></li>'
  );
  $('.circle-cpu')
    ...
    .on('circle-animation-progress', function (event, progress) {
      if (progress > data['cpu']) {
        $(this)
          .find('strong')
          .html(parseInt(100 * data['cpu']) + '%');
      } else {
        $(this)
          .find('strong')
          .html(parseInt(100 * progress) + '%');
      }
    });

  // Memory usage
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="circle circle-memory col-md-3 col-lg-3"><strong></strong><br/><span>' +
      MESSAGES[37] +
      '</span></div></li>'
  );
  $('.circle-memory')
  ...
  // Swap usage
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="circle circle-swap col-md-3 col-lg-3"><strong></strong><br/><span>Swap usage</span></div></li>'
  );
  $('.circle-swap')
    .circleProgress({
      arcCoef: 0.7,
  ...
  // Disk usage
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="circle circle-disk col-md-3 col-lg-3"><strong></strong><br/><span>' +
      MESSAGES[38] +
      '</span></div></li>'
  );
  $('.circle-disk')
  ...
  // IOL running nodes
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="count count-iol col-md-4 col-lg-4"></div>'
  );
  $('.count-iol', $statusModalBody).html(
    '<strong>' +
      data['iol'] +
      '</strong><br/><span>' +
      MESSAGES[41] +
  ...
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="count count-dynamips col-md-4 col-lg-4"></div></li>'
  );
  $('.count-dynamips', $statusModalBody).html(
    '<strong>' +
      data['dynamips'] +
      '</strong><br/><span>' +
      MESSAGES[42] +
  ...
  // QEMU running nodes
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="count count-qemu col-md-4 col-lg-4"></div></li>'
  );
  $('.count-qemu', $statusModalBody).html(
    '<strong>' +
      data['qemu'] +
      '</strong><br/><span>' +
      MESSAGES[43] +
  ...
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="count count-docker col-md-4 col-lg-6"></div></li>'
  );
  $('.count-docker', $statusModalBody).html(
    '<strong>' +
      data['docker'] +
      '</strong><br/><span>' +
      MESSAGES[161] +
  ...
  // VPCS running nodes
  $('#stats-graph ul', $statusModalBody).append(
    '<li><div class="count count-vpcs col-md-4 col-lg-6"></div></li>'
  );
  $('.count-vpcs', $statusModalBody).html(
    '<strong>' +
      data['vpcs'] +
      '</strong><br/><span>' +
      MESSAGES[162] +
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data['status'] == 'success') {
        logger(
          1,
          'DEBUG: got shape ' + id + 'from lab "' + lab_filename + '".'
        );

        try {
          data['data'].data = new TextDecoderLite('utf-8').decode(
            toByteArray(data['data'].data)
          );
        } catch (e) {
...
function createTextObject(newData) {
  ...
  if (newData.data) {
    newData.data = fromByteArray(
      new TextEncoderLite('utf-8').encode(newData.data)
    );
  }
...
function printFormEditCustomShape(id) {
  ...
  getTextObject(id).done(function (res) {
    var borderTypes = ['solid', 'dashed'],
      ...
      bgColor,
      html =
        '<form id="main-modal-edit" class="container col-md-12 col-lg-12 edit-custom-shape-form" data-path="' +
        id +
        '">' +
        '<div class="row col-md-9 top-part">' +
        '<label class="edit-custom-shape-label">Edit: ' +
        id +
        '</label><br>' +
        '<div class="form-group">' +
        '<label class="control-label form-group-addon">Z-Index</label>' +
        '<input type="number" class="form-control shape-z_index-input"  min="-100" max="100" step="1" data-path="' +
        id +
        '">' +
        '</div>' +
        '<div class="form-group">' +
        '<label class="control-label form-group-addon">Border-width</label>' +
        '<input type="number" class="form-control shape_border_width" min="0" data-path="' +
        id +
        '">' +
        '</div>' +
        '<div class="form-group">' +
        '<label class=" control-label form-group-addon">Border-type</label>' +
        '<select class="form-control border-type-select" data-path="' +
        id +
        '">' +
        '</select>' +
        '</div>' +
        ...
        '<input type="color" class="form-control shape_border_color" data-path="' +
        id +
        '">' +
        '</div>' +
        '<div class="form-group">' +
        '<label class="control-label form-group-addon">Background-color</label> ' +
        '<input type="color" class="form-control shape_background_color" data-path="' +
        id +
        '">' +
        '</div>' +
        '<div class="form-group">' +
        '<label class="control-label form-group-addon">Transparent</label> ' +
        '<button type="button" class="btn btn-default shape_background_transparent" data-path="' +
        id +
        '"></button>' +
        '</div>' +
        '<div class="form-group">' +
        '<label class="control-label form-group-addon">Rotate</label>' +
        '<input type="number" class="form-control shape-rotation-input" data-path="' +
        id +
        '" min="-360" max="360" step="1">' +
        '</div>' +
        '<div class="form-group">' +
        '<label class="control-label form-group-addon">Name</label>' +
        '<input type="text" class="form-control shape-name-input" data-path="' +
        id +
        '" >' +
        '</div>' +
        '</div>' +
        '<div class="row col-md-3 btn-part">' +
        '<button type="button" class="btn btn-success edit-custom-shape-form-save" data-path="' +
        id +
        '">' +
        MESSAGES[47] +
        '</button>' +
        '<button type="button" class="btn btn-flat cancelForm" data-path="' +
        id +
        '">' +
        MESSAGES[18] +
        '</button>' +
    ...
    for (var i = 0; i < borderTypes.length; i++) {
      $('.edit-custom-shape-form .border-type-select').append(
        $('<option></option>').val(borderTypes[i]).html(borderTypes[i])
      );
    }
    ...
    if (transparent) {
      $('.edit-custom-shape-form .shape_background_transparent')
    ...
    firstShapeValues['shape-background-color'] = rgb2hex(
      $('#customShape' + id + ' svg')
        .children()
        .attr('fill')
    );
    firstShapeValues['shape-border-color'] = rgb2hex(
      $('#customShape' + id + ' svg ')
        .children()
        .attr('stroke')
    );
    firstShapeValues['shape-border-width'] = $('#customShape' + id + ' svg')
      .children()
      .attr('stroke-width');
    ...
    // fill inputs
    $('.edit-custom-shape-form .shape-z_index-input').val(
      firstShapeValues['shape-z-index'] - 1000
    );
    $('.edit-custom-shape-form .shape_background_color').val(
      firstShapeValues['shape-background-color']
    );
    $('.edit-custom-shape-form .shape_border_color').val(
      firstShapeValues['shape-border-color']
    );
    $('.edit-custom-shape-form .shape_border_width').val(
      firstShapeValues['shape-border-width']
    );
    $('.edit-custom-shape-form .shape-rotation-input').val(
      firstShapeValues['shape-rotation']
    );
    $('.edit-custom-shape-form .shape-name-input').val(
      firstShapeValues['shape-name']
    );

    // fill backup
    $('.edit-custom-shape-form .firstShapeValues-z_index').val(
      firstShapeValues['shape-z-index']
    );
    $('.edit-custom-shape-form .firstShapeValues-border-color').val(
      firstShapeValues['shape-border-color']
    );
    $('.edit-custom-shape-form .firstShapeValues-background-color').val(
      firstShapeValues['shape-background-color']
    );
    $('.edit-custom-shape-form .firstShapeValues-border-type').val(
      firstShapeValues['border-types']
    );
    $('.edit-custom-shape-form .firstShapeValues-border-width').val(
      firstShapeValues['shape-border-width']
    );
    $('.edit-custom-shape-form .firstShapeValues-rotation').val(
      firstShapeValues['shape-rotation']
    );
    ...
    if (
      ...
      $('.edit-custom-shape-form .shape_border_width').val(
        firstShapeValues['shape-border-width'] * 2
      );
      $('.edit-custom-shape-form .firstShapeValues-border-width').val(
        firstShapeValues['shape-border-width'] * 2
      );
...
function printFormEditText(id) {
  ...
  var firstTextValues = {},
    ...
    bgColor,
    html =
      '<form id="main-modal-edit" class="container col-md-12 col-lg-12 edit-custom-text-form"  data-path="' +
      id +
      '">' +
      '<div class="row col-md-9  top-part">' +
      '<label class="edit-custom-text-label">Edit: ' +
      id +
      '</label><br>' +
      '<div class="form-group">' +
      '<label class="control-label form-group-addon">Z-Index</label>' +
      '<input type="number" class="form-control text-z_index-input" min="-100" max="100" step="1" data-path="' +
      id +
      '">' +
      '</div>' +
      '<div class="form-group">' +
      ...
      '<div class="btn-group">' +
      '<button type="button" class="btn btn-default btn-align-left" data-path="' +
      id +
      '"><i class="glyphicon glyphicon-align-left"></i></button>' +
      '<button type="button" class="btn btn-default btn-align-center" data-path="' +
      id +
      '"><i class="glyphicon glyphicon-align-center"></i></button>' +
      '<button type="button" class="btn btn-default btn-align-right" data-path="' +
      id +
      ...
      '<div class="btn-group">' +
      '<button type="button" class="btn btn-default btn-text-bold" data-path="' +
      id +
      '"><i class="glyphicon glyphicon-bold"></i></button>' +
      '<button type="button" class="btn btn-default btn-text-italic" data-path="' +
      id +
      '"><i class="glyphicon glyphicon-italic"></i></button>' +
      '</div>' +
      '</div>' +
      '<div class="form-group">' +
      '<label class="control-label form-group-addon">Text-color</label>' +
      '<input type="color" class="form-control text_color"  data-path="' +
      id +
      '">' +
      '</div>' +
      '<div class="form-group">' +
      '<label class="control-label form-group-addon">Background-color</label> ' +
      '<input type="color" class="form-control text_background_color"  data-path="' +
      id +
      '">' +
      '</div>' +
      '<div class="form-group">' +
      '<label class="control-label form-group-addon">Transparent</label> ' +
      '<button type="button" class="btn btn-default text_background_transparent" data-path="' +
      id +
      '"></button>' +
      '</div>' +
      '<div class="form-group">' +
      '<label class="control-label form-group-addon">Rotate</label>' +
      '<input type="number" class="form-control text-rotation-input"  data-path="' +
      id +
      '" min="-360" max="360" step="1">' +
      '</div>' +
      '<div class="form-group">' +
      ...
      '</div>' +
      '<div class="row col-md-3 btn-part">' +
      '<button type="button" class="btn btn-flat edit-custom-text-form-save" data-path="' +
      id +
      '">' +
      MESSAGES[47] +
      '</button>' +
  ...
  if (transparent) {
    $('.edit-custom-text-form .text_background_transparent')
      .addClass('active  btn-success')
      .text('On');
  } else {
    $('.edit-custom-text-form .text_background_transparent')
  ...
  firstTextValues['text-z-index'] = parseInt(
    $('#customText' + id).css('z-index')
  );
  firstTextValues['text-color'] = rgb2hex(
    $('#customText' + id + ' p').css('color')
  );
  firstTextValues['text-background-color'] = rgb2hex(
    $('#customText' + id + ' p').css('background-color')
  );
  firstTextValues['text-rotation'] = getElementsAngle('#customText' + id);

  $('.edit-custom-text-form .text-z_index-input').val(
    parseInt(firstTextValues['text-z-index']) - 1000
  );
  $('.edit-custom-text-form .text_color').val(firstTextValues['text-color']);
  $('.edit-custom-text-form .text_background_color').val(
    firstTextValues['text-background-color']
  );
  $('.edit-custom-text-form .text-rotation-input').val(
    firstTextValues['text-rotation']
  );
  ...
  $('.edit-custom-text-form .firstTextValues-z_index').val(
    parseInt(firstTextValues['text-z-index'])
  );
  $('.edit-custom-text-form .firstTextValues-color').val(
    firstTextValues['text-color']
  );
  $('.edit-custom-text-form .firstTextValues-background-color').val(
    $('#customText' + id + ' p').css('background-color')
  );
  $('.edit-custom-text-form .firstTextValues-italic').val(
    firstTextValues['text-type-italic']
  );
  $('.edit-custom-text-form .firstTextValues-bold').val(
    firstTextValues['text-type-bold']
  );
  $('.edit-custom-text-form .firstTextValues-align').val(
    firstTextValues['text-align']
  );
  $('.edit-custom-text-form .firstTextValues-rotation').val(
    firstTextValues['text-rotation']
  );
...
function printLogs(file, per_page, search) {
  ...
  $('#actions-menu').empty();
  $('#actions-menu').append(
    '<li><a href="#" onclick="search_log(); return false"><i class="glyphicon glyphicon-refresh"></i> Refresh</a></li>'
  );

  $.when(getLogs(file, per_page, search))
    .done(function (data) {})
    .fail(function (message) {
      addModalError(message);
    });
...
function lockLab() {
  ...
  for (var i = 0; i < allElements.length; i++) {
    if (toogleDruggable(lab_topology, allElements[i]))
  ...
  $('.action-lock-lab').html(
    '<i style="color:red" class="glyphicon glyphicon-remove-circle"></i>' +
      MESSAGES[167]
  );
  $('.action-lock-lab')
    .removeClass('action-lock-lab')
    .addClass('action-unlock-lab');
}
...
function unlockLab() {
  ...
  $('.action-unlock-lab').html(
    '<i class="glyphicon glyphicon-ok-circle"></i>' + MESSAGES[166]
  );
  $('.action-unlock-lab')
    .removeClass('action-unlock-lab')
    .addClass('action-lock-lab');
}
